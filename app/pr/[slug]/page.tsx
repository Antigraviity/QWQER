...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
    const post = await db.prPost.findUnique({
        where: { slug: params.slug },
        select: { title: true, excerpt: true, image: true, slug: true },
    });
    if (!post) return { title: 'Post Not Found | QWQER' };
    const url = `https://qwqer.in/pr/${post.slug}`;
    return {
        title: `${post.title} | QWQER Public Relations`,
        description: post.excerpt || `Read "${post.title}" — QWQER Public Relations.`,
        openGraph: { title: post.title, description: post.excerpt || '', url, type: 'article', ...(post.image && { images: [{ url: post.image }] }) },
        alternates: { canonical: url },
    };
}

export default async function PrPostPage({ params }: { params: { slug: string } }) {
    const post = await db.prPost.findUnique({ where: { slug: params.slug } });
    if (!post) notFound();
            ...
            <div className="max-w-[1400px] mx-auto px-6 pb-20 flex gap-8">
                <article className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-400 mb-8">
                        <span>{post.date}</span><span>•</span><span>{post.readTime}</span><span>•</span><span className="text-[#ee3425] font-medium">Public Relations</span>
                    </div>
                    {post.image && (
