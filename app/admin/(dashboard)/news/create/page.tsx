...
import { createNewsPost } from '@/lib/data-actions';
...
export default function CreateNewsPostPage() {
    ...
    const [state, dispatch] = useFormState(createNewsPost, initialState);
    ...
    return (
        <main className="max-w-4xl mx-auto">
            ...
            <form action={dispatch} className="bg-white p-6 md:p-8 rounded-xl border border-gray-200 shadow-sm space-y-6">
                ...
                <div><label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-2">Title</label><input id="title" name="title" type="text" placeholder="Enter post title" required className="w-full bg-white border border-gray-300 rounded-lg p-3 text-gray-900 placeholder-gray-400 focus:border-[#ee3425] focus:outline-none focus:ring-1 focus:ring-[#ee3425] shadow-sm" />{state.errors?.title?.map((e: string) => <p key={e} className="mt-2 text-sm text-red-500">{e}</p>)}</div>
                <div><label htmlFor="slug" className="block text-sm font-medium text-gray-700 mb-2">Slug</label><input id="slug" name="slug" type="text" ref={slugRef} placeholder="enter-post-slug" required className="w-full bg-white border border-gray-300 rounded-lg p-3 text-gray-900 placeholder-gray-400 focus:border-[#ee3425] focus:outline-none focus:ring-1 focus:ring-[#ee3425] shadow-sm" />{state.errors?.slug?.map((e: string) => <p key={e} className="mt-2 text-sm text-red-500">{e}</p>)}</div>
                <div>
